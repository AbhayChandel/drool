...
public class UserActivityRepositoryImpl implements UserActivityRepository {
    ...
    public UpdateResult addVideo(VideoDoc videoDoc) {
        ...
        return mongoOperations.upsert(query(where("userId").is(videoDoc.getUserRef().getId())), new Update().addToSet(arrayPath, new PostRef(videoDoc.getId(), videoDoc.getTitle(), null, null, videoDoc.getDatePosted())), UserActivityDoc.class);
    }
    ...
    public UpdateResult removeVideoComment(VideoCommentDto videoCommentDto) {
        Query queryUser = Query.query(Criteria.where("userId").is(videoCommentDto.getUserRefDto().getId()));
